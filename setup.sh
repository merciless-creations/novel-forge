...
preflight() {
  ...
  if [[ "$NF_OPENCODE" == "opencode" ]] && ! command -v opencode &>/dev/null; then
    log_error "OpenCode not found."
    log_error "Please install OpenCode first: https://opencode.ai"
    exit 1
  fi
  log_ok "OpenCode found at ${NF_OPENCODE}"
...
install_uv() {
  ...
  if [[ "$NF_UV" == "uv" ]] && ! command -v uv &>/dev/null; then
...
configure_api_keys() {
  ...
  # Generate environment file for systemd/launchd services
  # This ensures background services can access API keys without sourcing shell config
  generate_env_file

  echo ""
}

# ─── Helper: Generate environment file for services ─────────────────────────

generate_env_file() {
  local env_dir="${HOME}/.config/novel-forge"
  local env_file="${env_dir}/env"
  mkdir -p "$env_dir"

  # Write env file with restrictive permissions (owner-only read/write)
  {
    echo "# Novel Forge service environment — auto-generated by setup.sh"
    echo "# Do not edit manually; re-run setup.sh to regenerate."
    [[ -n "${OPENAI_API_KEY:-}" ]] && echo "OPENAI_API_KEY=${OPENAI_API_KEY}"
    [[ -n "${MEM0_API_KEY:-}" ]] && echo "MEM0_API_KEY=${MEM0_API_KEY}"
    echo "HOME=${HOME}"
    echo "PATH=${HOME}/.opencode/bin:${HOME}/.local/bin:/usr/local/bin:/usr/bin:/bin"
  } > "$env_file"

  chmod 600 "$env_file"
  log_ok "Service environment file created at ${env_file}"
}
...
setup_novels_repo() {
  ...
  # Ensure git identity is configured before any commits
  ...
  # opencode.json (generated with user-specific paths)
...
configure_git_identity() {
  local novels_dir="${NF_NOVELS_DIR}"

  # Check if git identity is configured (local or global)
  local has_name has_email
  has_name="$(git -C "$novels_dir" config user.name 2>/dev/null)" || true
  has_email="$(git -C "$novels_dir" config user.email 2>/dev/null)" || true

  if [[ -z "$has_name" || -z "$has_email" ]]; then
    echo ""
    log_info "Git needs your name and email for commit history."
    log_info "(This is stored locally in your novels repo only.)"
    echo ""

    if [[ -z "$has_name" ]]; then
      local git_name
      prompt_value "Your name (e.g., Jane Smith)" git_name
      if [[ -n "$git_name" ]]; then
        git -C "$novels_dir" config user.name "$git_name"
      fi
    fi

    if [[ -z "$has_email" ]]; then
      local git_email
      prompt_value "Your email" git_email
      if [[ -n "$git_email" ]]; then
        git -C "$novels_dir" config user.email "$git_email"
      fi
    fi
...
# Source API keys
if [[ -f "${HOME}/.config/novel-forge/env" ]]; then
  set -a
  source "${HOME}/.config/novel-forge/env" 2>/dev/null || true
  set +a
fi
...
WATCHEREOF
  ...
  # Copy the watcher script and its dependencies into the novels directory
  ...
  # Fix the source paths in the copied watcher to point to the .novel-forge copies
  sed_i "s|source \"\${SCRIPT_DIR}/platform.sh\"|source \"${NF_NOVELS_DIR}/.novel-forge/platform.sh\"|" \
    "${NF_NOVELS_DIR}/.novel-forge/watcher.sh"
  sed_i "s|source \"\${SCRIPT_DIR}/notify.sh\"|source \"${NF_NOVELS_DIR}/.novel-forge/notify.sh\"|" \
    "${NF_NOVELS_DIR}/.novel-forge/watcher.sh"
...
install_systemd_services() {
  ...
  cp "${SCRIPT_DIR}/services/novel-forge-watcher.service" "$watcher_unit"
  # Substitute user-specific values
  sed_i "s|NF_NOVELS_DIR=%h/novels|NF_NOVELS_DIR=${NF_NOVELS_DIR}|" "$watcher_unit"
  sed_i "s|NF_DROPZONE=%h/DropZone|NF_DROPZONE=${NF_DROPZONE}|" "$watcher_unit"
  sed_i "s|NF_OPENCODE_PORT=4096|NF_OPENCODE_PORT=${NF_OPENCODE_PORT}|" "$watcher_unit"
  log_ok "Watcher systemd unit installed"

  # OpenCode service
  local opencode_unit="${NF_SERVICE_DIR}/novel-forge-opencode.service"
  cp "${SCRIPT_DIR}/services/novel-forge-opencode.service" "$opencode_unit"
  # Substitute user-specific values
  sed_i "s|WorkingDirectory=%h/novels|WorkingDirectory=${NF_NOVELS_DIR}|" "$opencode_unit"
  sed_i "s|--port 4096|--port ${NF_OPENCODE_PORT}|" "$opencode_unit"
...
install_launchd_services() {
  ...
  # Watcher plist — replace __HOME__ placeholder with actual home directory
  local watcher_plist="${NF_SERVICE_DIR}/com.novel-forge.watcher.plist"
  sed "s|__HOME__|${HOME}|g" "${SCRIPT_DIR}/services/com.novel-forge.watcher.plist" > "$watcher_plist"
  # Substitute user-specific values
  sed_i "s|${HOME}/DropZone|${NF_DROPZONE}|g" "$watcher_plist"
  sed_i "s|${HOME}/novels|${NF_NOVELS_DIR}|g" "$watcher_plist"
  sed_i "s|<string>4096</string>|<string>${NF_OPENCODE_PORT}</string>|g" "$watcher_plist"
  log_ok "Watcher launchd plist installed"

  # OpenCode plist — replace __HOME__ placeholder with actual home directory
  local opencode_plist="${NF_SERVICE_DIR}/com.novel-forge.opencode.plist"
  sed "s|__HOME__|${HOME}|g" "${SCRIPT_DIR}/services/com.novel-forge.opencode.plist" > "$opencode_plist"
  # Substitute user-specific values
  sed_i "s|${HOME}/novels|${NF_NOVELS_DIR}|g" "$opencode_plist"
  sed_i "s|--port 4096|--port ${NF_OPENCODE_PORT}|g" "$opencode_plist"
  # Replace opencode path if user has it installed elsewhere
  if [[ "$NF_OPENCODE" != "${HOME}/.opencode/bin/opencode" ]]; then
    sed_i "s|${HOME}/.opencode/bin/opencode|${NF_OPENCODE}|g" "$opencode_plist"
  fi
...
finish() {
  ...
  # Offer to clean up the installer directory
  echo ""
  log_info "The installer files in ${SCRIPT_DIR} are no longer needed."
  log_info "You can safely delete them to save space."
  if prompt_yes_no "Delete the novel-forge installer directory now?"; then
    # Can't delete ourselves while running, so create a self-destruct
    log_info "The installer directory will be removed when this script exits."
    trap "rm -rf '${SCRIPT_DIR}'" EXIT
  else
    log_info "Keeping installer at ${SCRIPT_DIR}"
    log_info "Delete it anytime with: rm -rf ${SCRIPT_DIR}"
  fi
}
